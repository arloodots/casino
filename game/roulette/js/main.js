...
var getClosestTop = function() {
    ...
    getBestPageUrl = function(a) {
        ...
        if (a.err) try {
            ...
            g = d.document.referrer
        } else g = d.location.href;
        return g
    },
...
function CMain(a) {
    ...
    this.initContainer = function() {
        ...
        seekAndDestroy() ?
            l = new CPreloader : window.location.href = "http://www.codethislab.com/contact-us.html"
    };
    this.soundLoaded = function() {
        g++;
        g === e && (l.unload(), this.gotoMenu())
    };
...
function CGame(a) {
    ...
    this._resetTable = function() {
        ...
        p.reset();
        z.reset();
        .1 > p.getCredit() ? (e = -1, r.hideBlock(), D.show()) : (r.enableRebet(), this._setState(STATE_GAME_WAITING_FOR_BET));
        f++;
    ...
    this._generateWinLoss = function() {
        ...
        } else -1 === WIN_OCCURRENCE ? (c = NUMBERS_TO_BET - b, e = Math.floor(38 * Math.random())) : (c = WIN_OCCURRENCE, e = Math.floor(100 * Math.random()));
        if (g = e >= c ? !1 : !0) {
            do e = Math.floor(Math.random() * a.length), d = a[e].win; while (0 === d)
        } else {
            c = [];
            for (e = 0; e < NUMBERS_TO_BET; e++) c.push(e);
            do {
                if (0 === c.length) {
    ...
    this.onSpin = function() {
        ...
        0 !== p.getCurBet() && (p.getCurBet() < MIN_BET ? (x.show(TEXT_ERROR_MIN_BET), r.enableBetFiches(), r.enableSpin(!0)) : r.isBlockVisible() || (r.showBlock(), z.hide(), C.hide(), r.enableSpin(!1), $(s_oMain).trigger("bet_placed", p.getCurBet()), this._startRouletteAnim(), this._startBallSpinAnim(), this._setState(STATE_GAME_SPINNING),
...
function CInterface() {
    ...
    this._init = function() {
        ...
        d = y.height / 2 + 10;
        h = new CGfxButton(a, d, y, s_oStage);
        h.addEventListener(ON_MOUSE_UP, this._onExit, this);
        if (!1 === DISABLE_SOUND_MOBILE || !1 === s_bMobile) y = s_oSpriteLibrary.getSprite("audio_icon"), b = a - y.width / 2 - 10, l = y.height / 2 + 10,
    ...
    this.setMoney = function(a) {
        u.text = a.toFixed(2) + TEXT_CURRENCY
    };
...
function CNeighborsPanel() {
    ...
    this._init = function() {
        ...
        m.addChild(a);
        t = new createjs.Text(g + TEXT_CURRENCY, "20px " + FONT1, "#fff");
        t.textAlign = "center";
        t.x = 690;
        t.y =
            564;
...
function CGameOver() {
    ...
    this._init = function() {
        ...
        d.y = 400;
        b.addChild(d);
        g = new CTextButton(CANVAS_WIDTH / 2 + 170, 510, s_oSpriteLibrary.getSprite("but_bg"),
            TEXT_RECHARGE, FONT1, "#fff", 14, !1);
        g.addEventListener(ON_MOUSE_UP, this._onRecharge, this);
        b.addChild(g.getSprite());
...
function CCreditsPanel() {
    ...
    this._init = function() {
        ...
        n.y = CANVAS_HEIGHT / 2;
        f.addChild(n);
        k = new createjs.Text("Muhammad Iqbal", "24px Arial", "#ffffff");
        k.x = CANVAS_WIDTH / 2;
        k.y = 440;
        k.textAlign = "center";
        f.addChild(k)
...
function CRollingTextController(a, d, g, e, b, l, k) {
    ...
    this.update = function() {
        ...
        if (f > h) f = 0, x.text = c.toFixed(2) + p, null !== A && (A.text = c.toFixed(2) + p), clearInterval(u), null !== m[ON_CONTROLLER_END] && void 0 !== m[ON_CONTROLLER_END] && m[ON_CONTROLLER_END].call(r[ON_CONTROLLER_END], this), 0 < w ? this.setUpdateInfo(w) : null !== m[ON_CONTROLLER_REMOVE] && void 0 !== m[ON_CONTROLLER_REMOVE] && m[ON_CONTROLLER_REMOVE].call(r[ON_CONTROLLER_REMOVE], this);
            ...
            x.text = a.toFixed(2) + p;
            null !== A && (A.text = a.toFixed(2) + p)
        }
    };
